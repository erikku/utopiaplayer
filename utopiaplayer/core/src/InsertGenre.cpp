...
InsertGenre::InsertGenre(QWidget* parent) : QWidget(parent)
{
...
void InsertGenre::insert()
{
	QSqlQuery query;
	query.prepare( QString::fromUtf8("INSERT INTO `un_genres` (`ジャンル`, `Genre`) VALUES (?, ?)") );
	query.addBindValue( QVariant(ui.KanjiGenreEdit->text()) );
	query.addBindValue( QVariant(ui.RomajiGenreEdit->text()) );
	if( query.exec() )
	{
		close();
		emit updateDatabase();
