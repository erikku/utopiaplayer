...
InsertAlbum::InsertAlbum(QWidget* parent) : QWidget(parent)
{
	setWindowModality(Qt::WindowModal);
	setWindowFlags(Qt::Dialog);
	setFixedSize(611, 211);
...
void InsertAlbum::refreshCover(const QString & RelativeCoverPath)
{
	QFile file(mCoversDirectory + RelativeCoverPath);
	if(file.exists())
	{
		QPixmap cover(mCoversDirectory + RelativeCoverPath);
		ui.AlbumCover->setPixmap( cover.scaledToWidth( 151, Qt::SmoothTransformation ).scaledToHeight( 151, Qt::SmoothTransformation ) );
		ui.AlbumCover->show();
		//ui.AlbumCover->setPixmap( cover.scaled ( 151, 151, Qt::KeepAspectRatio, Qt::SmoothTransformation ) );
	}
};

void InsertAlbum::refreshOptions()
{
	QSettings settings;
